...
object BaseStationProducer {
  def main(args: Array[String]): Unit = {
    val sparkConf = new SparkConf().setAppName("BaseStationProducer").setMaster("local[*]")
    val ssc = new SparkContext(sparkConf)
    ssc.textFile(this.getClass.getResource("/snl_dim.dim_m_cell_adj_all_20201207.txt").getPath)
